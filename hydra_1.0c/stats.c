 ...
 *
 * $Id: stats.c,v 3.1 1998/04/17 16:51:35 skadron Exp $
 *
 * $Log: stats.c,v $
 ...
 *
 * Revision 1.2  1997/03/04  02:42:12  skadron
 * Fixed average, stdev, and imax for print_dist()
 *
...
{
  unsigned int i, bbins, imax;
  double btotal, bwtotal, bvar, bavg;
  int pf = stat->variant.for_dist.pf;

  /* count and sum entries */
  bbins = 0; btotal = 0.0; bwtotal = 0.0; bvar = 0.0;
  imax = 0;
  for (i=0; i<stat->variant.for_dist.arr_sz; i++)
    {
      bbins++;
      bwtotal += stat->variant.for_dist.arr[i] * (double)i;
      btotal += stat->variant.for_dist.arr[i];
      if (stat->variant.for_dist.arr[i] != 0.0)
	imax = i;
    }

  /* Compute average and stdev */
  bavg = bwtotal / (btotal != 0.0 ? btotal : 1);
  for (i=0; i<stat->variant.for_dist.arr_sz; i++)
    bvar += (((double)i - (double)bavg) * ((double)i - (double)bavg)) *
	    stat->variant.for_dist.arr[i];
  bvar /= ((btotal - 1) > 0.0) ? (btotal - 1) : 1;
  ...
  fprintf(fd, "%s.imin = %u\n", stat->name, 0U);
  fprintf(fd, "%s.imax = %u\n", stat->name, 
	  imax * stat->variant.for_dist.bucket_sz);
...
/* FIXME: Haven't fixed averages, stdevs, etc here --Ks */
static void
