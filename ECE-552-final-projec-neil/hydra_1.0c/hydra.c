...
"      -cache:dl1 ul1:256:32:1:l:4:2 -cache:dl2 ul2:1024:64:2:l:4:4\n"
  ...
  opt_reg_uint(odb, "-sim_insts",
	      ...
	      "after priming", &num_fullsim_insn, /* default */0,
	      /* print */TRUE, /* format */NULL);
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  if (max_threads > N_THREAD_RECS)
    fatal("max number of threads can't currently exceed %d.  \n"
	  "   Try increasing N_THREAD_RECS", N_THREAD_RECS);
  ...
  if (max_threads > 1
      && bconf_type != BCF_Naive && bconf_type != BCF_Omni)
    {
  ...
  /* boosting */
  if (pred_synth_up_threshold < 0.0 || pred_synth_up_threshold > 1.0
      || pred_synth_down_threshold < 0.0 || pred_synth_down_threshold > 1.0)
  ...
  if (perfect_except_retstack 
      && (!fix_addrs_indir || !fix_addrs || pred_synth_up_threshold < 1.0))
...
sum_priorities(void)
{
  ...
  for (t = 0; t < N_THREAD_RECS; t++)
	...
	&& thread_info[t].fetchable <= sim_cycle)
...
int
ruu_occ_cmp(const void *el1, const void *el2)
{
...
num_fetchable_threads()
{
  int t, num = 0;
  for (t = 0; t < N_THREAD_RECS; t++)
    if (thread_info[t].fetchable <= sim_cycle 
	&& thread_info[t].valid == TRUE
	&& thread_info[t].squashed != TRUE
	&& (ruu_include_spec || thread_info[t].spec_mode != TRUE))
      num++;
...
get_next_fetch_thread(int *pri, int go_backwards)
{
  ...
  if (fetch_pri_pol == Ruu_Pri)
      ...
      while (next_by_ruu_thread < N_THREAD_RECS)
	{
	  ...
	  if (thread_info[best_candidate].valid == TRUE
	      && thread_info[best_candidate].squashed != TRUE
	      && !thread_info[best_candidate].fetched_this_cycle
	      && thread_info[best_candidate].fetchable <= sim_cycle
	      && (ruu_include_spec 
		  || thread_info[best_candidate].spec_mode != TRUE))
	    {
  ...
  switch (fetch_pri_pol)
    ...
    case Pred_Pri2:
      ...
      t = (last_thread_fetched + (go_backwards ? N_THREAD_RECS-1 : 1)) 
	  % N_THREAD_RECS;  
      break;
  ...
  for (i = 0 ; 
       i < N_THREAD_RECS ; 
       t = (t + 
	        (((fetch_pri_pol==Pred_Pri2 || fetch_pri_pol==Pred_RR) 
		  && go_backwards)
		 ? N_THREAD_RECS-1 
		 : 1))
	   % N_THREAD_RECS, 
	...
	&& thread_info[t].valid == TRUE
	&& thread_info[t].squashed != TRUE
	&& (!thread_info[t].fetched_this_cycle || fetch_pri_pol == Simple_RR 
	    || fetch_pri_pol == Pred_RR)
	&& (ruu_include_spec || thread_info[t].spec_mode != TRUE))
      {
	switch(fetch_pri_pol)
	  {
	  ...
	  case Pred_Pri2:
	    ...
	    dassert(thread_info[t].base_priority == -1
		    && thread_info[t].priority == -1);
	    last_thread_fetched = t;
	    if (fetch_pri_pol != Simple_RR && fetch_pri_pol != Pred_RR
		&& fetch_pri_pol != Pred_Pri2)
	  ...
	  case Pred_Pri:
	    ...
	    if (thread_info[t].priority > largest_pri)
	      {
		assert(thread_info[t].priority >= 0 
		       && thread_info[t].base_priority > 0);
  ...
  if (!aggressive_issue
	  ...
	  && !(rs->in_LSQ || (SS_OP_FLAGS(rs->op) & (F_LONGLAT|F_CTRL)))))
    {
      /* FIXME: for multi-path; this means priority is by fetch order */
			...
			int threaddepth = thread_info[rs->thread_id].fork_hist_bmap_ptr;
			if(threaddepth<fork_hist_bmap_head) threaddepth+=N_SPEC_LEVELS;
			int curdepth = 0;
			for (;node;
	   prev=node, node=node->next){
			 curdepth = thread_info[node->rs->thread_id].fork_hist_bmap_ptr;
			 if(curdepth<fork_hist_bmap_head) curdepth+=N_SPEC_LEVELS;
				if(curdepth>=threaddepth) break;
		 }
     //NEIL WAS HERE: get to the first node such that its speculation level is equal to or greater than the current one
    ...
    } //NEIL WAS HERE: keep going until it finds its place in fetch order among instructions of the same depth
	  ...
	  if (!rs->squashed && ptrace_level != PTRACE_FUNSIM)
	    {
	      ptrace_newstage(LSQ[LSQ_head].ptrace_seq, 
			      ...
			      PST_COMMIT, 
			      events 
			        | (LSQ[LSQ_head].spec_mode ?PEV_SPEC_MODE:0));
		    ...
		    char *caller_string) /* caller of this func, f/ printouts*/
{
  ...
  if (per_thread_retstack == OneStackPredOnly
      && (fetch_pri_pol == Pred_RR || fetch_pri_pol == Pred_Pri2)
      && curr_thread != pred_thread)
    gate = FALSE;
  ...
  if (per_thread_retstack == PerThreadTOSP)
	...
	{
	  thread_info[curr_thread].retstack_tos 
	    = recover_rec->tos;
...
ruu_recover(int branch_index)			/* index of mis-pred branch */
{
  ...
  while (RUU_index != branch_index)
      ...
      if (RUU[RUU_index].squashable)
	{
	  ...
	  if (pred->retstack.patch_level == RETSTACK_PATCH_WHOLE
	      && RUU[RUU_index].bpred_recover_rec.contents.stack_copy)
	    {
...
ruu_writeback(void)
{
  ...
  for (i = 0, curr = RUU_head; i < RUU_num; i++, curr = (curr + 1) % RUU_size)
	...
	break;
      ...
      if ((SS_OP_FLAGS(RUU[curr].op) & (F_CTRL|F_COND)) == (F_CTRL|F_COND)
	  && !RUU[curr].completed)
  ...
  while ((rs = eventq_next_event()))
      ...
      if (SS_OP_FLAGS(rs->op) & F_CTRL)
	{
	  ...
	  if (rs->forked && (fork_prune || fetch_pred_pri))
	    {
	      ...
	      if (rs->token2forked 
		  && thread_info[rs->forked_thread].valid == KILLED
		  && thread_info[rs->thread_id].valid == TRUE)
		thread_info[rs->thread_id].pred_path_token = TRUE;
	      else if (!rs->token2forked 
		       && thread_info[rs->thread_id].valid == KILLED
		       && thread_info[rs->forked_thread].valid == TRUE)
	  ...
	  if (rs->forked
	      && (fetch_pri_pol == Pred_RR || fetch_pri_pol == Pred_Pri2)
	      && rs->new_pred_path_token
	      && rs->squashed != TRUE
	      && rs->br_taken != rs->br_pred_taken)
	    {
		...
		new_token_holder = rs->nottaken_thread;
	      ...
	      for (i = (rs-RUU+1) % RUU_size, n = 0; 
		   i != RUU_tail;
		   i = (i+1) % RUU_size, n++)
		if ((SS_OP_FLAGS(RUU[i].op) & F_CTRL)
		    && RUU[i].thread_id == new_token_holder
		    && RUU[i].squashed != TRUE)
		  {
	      ...
	      dassert(pred_thread >= 0 && pred_thread < N_THREAD_RECS);
#ifdef DEBUG_PRED_PRI
	      thread_info[new_token_holder].new_pred_path_token
		= ++new_pred_path_token_val;
	      assert(last_branch_seen->new_pred_path_token == TRUE);
#endif
	    }
	  else if ((rs->br_taken != rs->br_pred_taken
		    || ((SS_OP_FLAGS(rs->op) & (F_UNCOND))
			&& rs->used_PC != rs->next_PC))
		   && rs->squashed != TRUE
		   && rs->new_pred_path_token
		   && (fetch_pri_pol == Pred_RR || fetch_pri_pol == Pred_Pri2))
	    {
...
#ifdef DEBUG_PRED_PRI
	      thread_info[rs->thread_id].new_pred_path_token
		= ++new_pred_path_token_val;
#endif	      
	    }
	...
	ptrace_newstage(rs->ptrace_seq, rs->thread_id, PST_WRITEBACK,
			/* events */
			((rs->recover_inst || rs->forked) ? PEV_MPDETECT : 0)
      ...
      if (rs->l1_miss && !rs->spec_mode && report_miss_indep_insts)
	{
	  ...
	  for (j = 0, curr = RUU_head; 
	    ...
	    {
	      if (!RUU[curr].flag && !RUU[curr].spec_mode
		  && (RUU[curr].issued_at > rs->issued_at
		      || RUU[curr].queued))
		indep_insts++;
	      RUU[curr].flag = FALSE;
	    }
	  ...
	  if (rs->onames[i] != NA && !rs->squashed)
	      ...
	      for (t=0; t<N_THREAD_RECS; t++)
		{
		  ...
		  for (s=0; s<=thread_max_spec_level; s++)
		  {
			...
			spec_create_vector[t][s][rs->onames[i]] 
			  = CVLINK_NULL;
			spec_create_vector_rt[t][s][rs->onames[i]] 
			  = sim_cycle;
		      }
...
lsq_refresh(void)
{
  ...
  for (i=0, index=LSQ_head, n_std_unknowns=0;
	  ...
	  (SS_OP_FLAGS(LSQ[index].op) & (F_MEM|F_STORE)) == (F_MEM|F_STORE))
	{
      ...
      if (/* load? */(SS_OP_FLAGS(LSQ[index].op) & F_LOAD)
...
ruu_issue(void)
{
  ...
  for (n_issued = 0, n_issued_useful = 0, n_int_issued = 0, n_fp_issued = 0; 
	 ...
	 && n_int_issued < ruu_int_issue_width
	 && n_fp_issued < ruu_fp_issue_width;
       node = next_node)
    {
      next_node = node->next;
      rs = RSLINK_RS(node);
      ...
      if (RSLINK_VALID(node) && !rs->squashed)
	{
	  ...
	  if (rs->in_LSQ
	      ...
	      if (SS_OP_FUCLASS(rs->op) != NA)
		{
		  ...
		  if (fu || infinite_fu)
		      ...
		      else /* !load && !store */
			{
			  ...
			  eventq_queue_event(rs, sim_cycle + oplat
					         + extra_issue_lat);
	      ...
	      else /* does not require a functional unit! */
		{
	      ...
	      {
		issue_depth=((long)rs-(long)&RUU[0])/sizeof(struct RUU_station)
		            - RUU_head;
		if (issue_depth < 0)
...
kill_threads(void)
{
  ...
  for (t = 0, n = 0, n2 = 0; t < N_THREAD_RECS; t++)
      ...
      if (thread_info[t].valid == TRUE)
	{
	  ...
	  if (fork_prune || fetch_pred_pri)
	    {
	      if (thread_info[t].pred_path_token 
		  && thread_info[t].spec_mode == FALSE
		  && saw_token)
       ...
       * probably orphaned */
      if (thread_info[t].valid 
	  && thread_info[t].fetchable + DEAD_TIME < sim_cycle)
  ...
  if ((fork_prune || fetch_pred_pri) 
      && sim_cycle - last_true_token_seen > DEAD_TIME)
    panic("lost prune-token!");
...
post_thread_cleanup(struct RUU_station *rs_branch)
{
  ...
  if (!rs_branch->forked)
    ...
    assert(thread_info[branch_thread].squashed != TRUE
	   || thread_info[branch_thread].valid != TRUE);
#endif
}
					       ...
					       * cleaning up IFQ */
{
  ...
  for (i = (ifq_tail + (ruu_ifq_size - 1)) % ruu_ifq_size, n = 0; 
      ...
      if (thread_info[ ifq[i].thread ].squashed == TRUE) 
	{
	  ifq[i].valid = FALSE;
	  if (pred->retstack.patch_level == RETSTACK_PATCH_WHOLE
	      && ifq[i].bpred_recover_rec.contents.stack_copy)
	    {
...
fork_thread(SS_ADDR_TYPE newPC, int forking_thread)
{
  ...
  if (fetch_pri_pol == Omni_Pri || fetch_pri_pol == Two_Omni_Pri
      || fetch_pri_pol == Pred_Pri || fetch_pri_pol == Ruu_Pri)
    {
  ...
  thread_info[new_thread].retstack_tos 
    = thread_info[forking_thread].retstack_tos;
...
void
thread_refork(int forked_thread, 
	      int forking_thread,
	      ...
	      int ruu_refork_penalty)
{
  ...
  if (thread_info[forked_thread].valid == KILLED)
    return;
  dassert(thread_info[forked_thread].squashed != TRUE 
  ...
  thread_info[forked_thread].retstack_tos 
    = thread_info[forking_thread].retstack_tos;
  if (per_thread_retstack == PerThreadStacks)
    {
      dassert(retstack_size);
  ...
  thread_info[forked_thread].fork_hist_bmap_ptr 
    = (forking_branch_bmap_ptr + 1) % N_SPEC_LEVELS;
		...
		int nbytes)			/* number of bytes to access */
{
  ...
  for (prev=NULL,ent=store_htable[index]; ent; prev=ent,ent=ent->next)
    {
      if (ent->addr == addr 
	  && ent->thread == thread && ent->spec_level == spec_level)
	{
...
#define SET_GPR(N,EXPR)      ((spec_mode == TRUE)                             \
			      ? (void)(spec_regs_R[curr_thread][spec_level][N]\
				       = (EXPR))                              \
			      : (regs_R[N] = (EXPR)))
...
#define SET_FPR_L(N,EXPR)  ((spec_mode == TRUE)                               \
			    ? (void)(spec_regs_F[curr_thread][spec_level].l[N]\
				     = (EXPR))                                \
			    : (regs_F.l[(N)] = (EXPR)))
...
#define SET_FPR_F(N,EXPR)  ((spec_mode == TRUE)                               \
			    ? (void)(spec_regs_F[curr_thread][spec_level].f[N]\
				     = (EXPR))                                \
			    : (regs_F.f[(N)] = (EXPR)))
...
void 
get_reg_contents(int regnum,                 /* result reg's "dep number" */
 ...
 * in in_vals */
void 
get_operands(int in1, int in2, int in3,       /* operand regs' "dep numbers" */
	     int curr_thread,                 /* current thread */
	     enum ss_opcode op,               /* this inst's opcode */
 ...
 * in out_vals */
void 
get_results(int out1, int out2,              /* result regs' "dep numbers" */
	    int curr_thread,                 /* current thread */
	    enum ss_opcode op,               /* this inst's opcode */
...
ruu_dispatch(void)
{
  ...
  while (/* instruction decode B/W left? */
	...
	goto empty_fetch_entry;
      ...
      if (!ruu_include_spec 
	  && (thread_info[ifq[ifq_head].thread].spec_mode == TRUE))
	{
      ...
      else /* Omni or fork-in-dispatch */
	{
      ...
      /* sanity checks */
      assert(thread_info[curr_thread].valid == TRUE
	     && thread_info[curr_thread].squashed == UNKNOWN);
#ifdef DEBUG
      if (forked)
      ...
      if (SS_OP_FLAGS(op) & F_TRAP)
	{
      ...
      regs_R[0] = 0; spec_regs_R[curr_thread][spec_level][0] = 0;
...
#define PATCH_NOT_TAKEN 6
	  ...
	  || ((SS_OP_FLAGS(op) & (F_CTRL|F_DIRJMP)) == (F_CTRL|F_DIRJMP) 
	      && PC_if_taken != pred_PC && br_pred_taken
	      && (patch_type = PATCH_MISFETCH))
	  /* or, misfetch on an indirect branch (used only to make
	   * indir branches always hit the BTB to measure the impact
	   * of their BTB misses) */
	  || ((SS_OP_FLAGS(op) & (F_CTRL|F_INDIRJMP)) == (F_CTRL|F_INDIRJMP) 
	      && pred_PC != next_PC
	      && fix_addrs_indir && (patch_type = PATCH_BTB_IJMP))
	  /* or, direction-mispredict under synthetic-boosted bpred */
	  || ((SS_OP_FLAGS(op) & (F_CTRL|F_COND)) == (F_CTRL|F_COND) 
	      && br_taken != br_pred_taken
	      && pred_synth_up && (patch_type = PATCH_SYNTH_UP))
	  || ((SS_OP_FLAGS(op) & (F_CTRL|F_COND)) == (F_CTRL|F_COND) 
	      && br_taken == br_pred_taken
	      && pred_synth_down && (patch_type = PATCH_SYNTH_DOWN))
	  || (pred_PC != next_PC && (pred->class == BPredNotTaken || !pred)
	      && (SS_OP_FLAGS(op) & (F_DIRJMP|F_INDIRJMP))
	      && !(SS_OP_FLAGS(op) & F_COND)
	      && br_taken && (patch_type = PATCH_NOT_TAKEN)))
	{
	  ...
	  if (pred_synth_down && br_taken == br_pred_taken
	      && (SS_OP_FLAGS(op) & F_COND))
	    {
	      ...
	      if (ranval < pred_synth_down_threshold)
		{
		  ...
		  dassert(patch_PC != next_PC
			  || PC_if_taken == regs_PC + SS_INST_SIZE);
		  penalty = 0;
		  goto dump_ifq;
		}
	  ...
	  if (pred_synth_up && br_taken != br_pred_taken
	      && (SS_OP_FLAGS(op) & F_COND))
	    {
	  ...
	  dassert(pred_perfect || br_pred_taken || patch_type==PATCH_BTB_IJMP
		  || patch_type == PATCH_NOT_TAKEN);
	   ...
	   * was correct */
	  if (perfect || (fix_addrs && br_taken) 
	      || patch_type == PATCH_BTB_IJMP)
   	    {
	  ...
	  dassert(br_taken == br_pred_taken || patch_PC != next_PC 
		 || pred_perfect
		 || patch_type==PATCH_BTB_IJMP || patch_type==PATCH_NOT_TAKEN);
 ...
 normal_branches:
      ...
      if (forked && predPC_if_taken != PC_if_taken)
	{
       ...
       */
      else if (bconf_type == BCF_Omni
	   && /* if approx'ing fork-in-fetch, resources avail at fetch time */
	      (could_have_forked > 0 || !approx_fork_in_fetch)
	   && /* resources still avail */num_active_forks < max_threads-1
	   && /* this branch mispredicted */patch_PC != next_PC
	   && /* only fork for cond br's */(SS_OP_FLAGS(op) & F_COND)
	   && /* only fork from the correct path */spec_mode != TRUE)
	{
	  ...
	  if (forked_thread >= 0)
		...
		= (thread_info[curr_thread].fork_hist_bmap_ptr + 1) 
		  % N_SPEC_LEVELS;
	      ...
	      else
		{
		  ...
		  (void)BITMAP_CLEAR(thread_info[curr_thread].fork_hist_bmap, 
				  THREADS_BMAP_SZ, 
				  thread_info[curr_thread].fork_hist_bmap_ptr);
		  penalty = ruu_fork_penalty 
		            + (fetch_redirected ? ruu_branch_penalty : 0);
	       ...
	       && /* only fork from the correct path */spec_mode != TRUE)
	{
	  ...
	  conf = LowConf;
	       ...
	       && /* only consider cond br's */(SS_OP_FLAGS(op) & F_COND))
	{
      ...
      else if (!fork_in_fetch && bconf_type != BCF_Omni)
	{
	  if (/* if approx'ing fork-in-fetch, resources avail at fetch time */
	         (could_have_forked > 0 || !approx_fork_in_fetch)
	      && /* resources still avail */num_active_forks < max_threads-1
	      && /* low-confidence branch or semi-omni fork-mispred */
	         (conf == LowConf 
		  || (patch_PC!=next_PC && !spec_mode && bconf_fork_mispred))
	      && /* only fork for cond br's */(SS_OP_FLAGS(op) & F_COND))
	    {
	      ...
	      if (!bconf_squash_extra 
		  || (patch_PC!=next_PC && !spec_mode))
		{
		  if (thread_info[curr_thread].pred_path_token || !fork_prune)
		    {
		      forked_thread = fork_thread(fork_PC, curr_thread);
	      ...
	      if (forked_thread >= 0)
		{
		  ...
		  if (conf == HighConf)
		    {
		      assert(patch_PC != next_PC && !spec_mode 
			     && bconf_fork_mispred);
		      total_extra_forked++;
		    }
	      ...
	      dassert(base_pred_PC == pred_PC);
		    ...
		    (could_have_forked && approx_fork_in_fetch)
		  && /*resources not now avail*/num_active_forks>=max_threads-1
		  && /* low-confidence branch or semi-omni fork-mispred */
		    (conf == LowConf 
		    || (patch_PC!=next_PC && !spec_mode && bconf_fork_mispred))
		  && /* only fork for cond br's */(SS_OP_FLAGS(op) & F_COND))
...
/* fork-in-fetch and pruning not supported */
      ...
      else if ((fetch_pri_pol==Pred_RR || fetch_pri_pol==Pred_Pri2) && forked
	       && pred_thread == curr_thread)
	{
#ifdef DEBUG_PRED_PRI
	  assert(thread_info[curr_thread].new_pred_path_token 
		  == new_pred_path_token_val);
#endif
	  ...
	  if (!fork_in_fetch)
...
#ifdef DEBUG_PRED_PRI
	      thread_info[curr_thread].new_pred_path_token 
		= ++new_pred_path_token_val;
#endif
	      pred_thread = curr_thread;
		...
		{
#ifdef DEBUG_PRED_PRI
		  thread_info[forked_thread].new_pred_path_token 
		    = ++new_pred_path_token_val;
#endif		      
      ...
      if (spec_mode != TRUE)
	{
	  ...
	  if (SS_OP_FLAGS(op) & F_CTRL)
		  ...
		  (SS_OP_FLAGS(rs->op) & F_COND))
		{
		  ...
		  int br_pred_taken = (base_pred_PC  
				       != (regs_PC + sizeof(SS_INST_TYPE)));
      ...
      else if (patch_PC != next_PC)
	{
	  ...
	  if (forked)
	    {
	      ...
	      memcpy(&spec_regs_F[forked_thread][spec_level], 
		     ...
		     sizeof(union regs_FP));
	      spec_regs_HI[forked_thread][spec_level] 
		= spec_regs_HI[curr_thread][spec_level];
	      spec_regs_LO[forked_thread][spec_level] 
		= spec_regs_LO[curr_thread][spec_level];
	      spec_regs_FCC[forked_thread][spec_level] 
		= spec_regs_FCC[curr_thread][spec_level];
	  ...
	  memcpy(&spec_regs_F[curr_thread][spec_level], 
		 ...
		 sizeof(union regs_FP));
	  spec_regs_HI[curr_thread][spec_level]
	    = spec_regs_HI[curr_thread][old_spec_level];
	  spec_regs_LO[curr_thread][spec_level] 
	    = spec_regs_LO[curr_thread][old_spec_level];
	  spec_regs_FCC[curr_thread][spec_level]
	    = spec_regs_FCC[curr_thread][old_spec_level];
	  ...
	  for (i=0; i<STORE_HASH_SIZE; i++)
	    {
	      ...
	      for (ent=store_htable[i]; ent; ent=ent->next)
		{
		  if (ent->thread != curr_thread 
		      || ent->spec_level != old_spec_level)
	  ...
	  if (forked)
	    {
	      ...
	      memcpy(&spec_regs_F[forked_thread][new_spec_level], 
		     &spec_regs_F[curr_thread][spec_level],
		     sizeof(union regs_FP));
	      spec_regs_HI[forked_thread][new_spec_level] 
		= spec_regs_HI[curr_thread][spec_level];
	      spec_regs_LO[forked_thread][new_spec_level] 
		= spec_regs_LO[curr_thread][spec_level];
	      spec_regs_FCC[forked_thread][new_spec_level] 
		= spec_regs_FCC[curr_thread][spec_level];
	...
	assert(rs->recover_inst == FALSE);
      assert(sim_num_cond_branches == num_forked_good + num_forked_wasted 
	                              + num_unforked_bad + num_unforked_good);
      assert(sim_total_cond_branches == total_forked_good+total_forked_wasted 
	                              +total_unforked_bad+total_unforked_good);
#endif
	  ...
	  if (SS_OP_FLAGS(op) & F_MEM)
	    {
	      ...
	      lsq->conf = conf;  dassert(lsq->conf == HighConf);
	      lsq->squashed = FALSE;
	...
	ptrace_newstage_verbose(pseq, curr_thread, PST_DISPATCH,
				/* events */
				(fetch_redirected ? PEV_MFOCCURED : 0)
				| ((!forked && (patch_PC != next_PC)) 
				   ? PEV_MPOCCURED : 0)
				| (spec_mode ? PEV_SPEC_MODE : 0),
...
fetch_dump(FILE *stream)			/* output stream */
{
  ...
  fprintf(stream, "** fetch stage state ** current cycle: %.0f **\n",
	  (double)sim_cycle);
  if (fetch_pri_pol != Simple_RR && fetch_pri_pol != Old_RR
      && fetch_pri_pol != Pred_RR && fetch_pri_pol != Pred_Pri2)
							     ...
							     * recovery info */
{
   ...
   * is a function call */
  if (per_thread_retstack == PerThreadStacks 
      && /* call? */(op == JAL || (op == JALR && jalr_r31p)))
    {
      dassert(retstack_size && thread_info[thread].retstack
			  && thread_info[thread].retstack_tos >= 0
			  && thread_info[thread].retstack_tos < retstack_size);

      thread_info[thread].retstack_tos 
	= (thread_info[thread].retstack_tos + 1) % retstack_size;
  ...
  if (per_thread_retstack==PerThreadStacks && op==JR && jr_r31p)
	...
	fprintf(stderr, "\t%d: 0x%x\n", 
      ...
      thread_info[thread].retstack_tos 
	= (thread_info[thread].retstack_tos + retstack_size - 1)
	% retstack_size;
    }
	  ...
	  int *p_num_fetched)		/* # insts fetched */
{
  ...
  /* Sanity checks */
  assert(thread_info[thread].valid == TRUE 
	 && thread_info[thread].squashed == UNKNOWN);
  ...
  if (!done && (!last_inst_missed || last_inst_tmissed))
    {
      ...
      if (ld_text_base <= fetch_pred_PC
	  && fetch_pred_PC < (ld_text_base+ld_text_size)
	  && !(fetch_pred_PC & (sizeof(SS_INST_TYPE)-1)))
	{
  ...
  if (max_in_flight_branches)
	...
	{
	  if ((SS_OP_FLAGS(RUU[curr].op) & F_CTRL)
	      && RUU[curr].thread_id == thread) /*  && !RUU[curr].completed) */
	    in_flight_branches++;
	}
	...
	{
	  if ((SS_OP_FLAGS(SS_OPCODE(ifq[curr].IR)) & F_CTRL)
	      && ifq[curr].thread == thread && ifq[curr].valid)
	    in_flight_branches++;
	}
  ...
  for (i = 0;
	...
	* can be fetched in one cycle; <= cache line size) */
	...
	{
	  if ((SS_OP_FLAGS(op) & F_CTRL)
	      && in_flight_branches >= max_in_flight_branches)
	    {
	      done = TRUE;
      ...
      if (pred)
	{
	  ...
	  if (SS_OP_FLAGS(op) & F_CTRL)
		...
		{
		  retstack_copy_rec.contents.stack_copy 
		    = calloc(retstack_size, sizeof(struct bpred_btb_ent));
		  if (!retstack_copy_rec.contents.stack_copy)
	      ...
	      ifq[ifq_tail].predPC_if_taken 
		= b_update_rec.predPC_if_taken;
      ...
      else
	{
      ...
      BITMAP_COPY(ifq[ifq_tail].fork_hist_bmap, 
		  ...
		  THREADS_BMAP_SZ);
      ifq[ifq_tail].fork_hist_bmap_ptr 
      ...
      if (forked)
	{
	  assert(fork_in_fetch);
	  thread_info[thread].fork_hist_bmap_ptr 
	    = (thread_info[thread].fork_hist_bmap_ptr + 1) % N_SPEC_LEVELS;
...
ruu_fetch_wrapper(void)
{
  ...
  if (fetch_pri_pol == Pred_RR || fetch_pri_pol == Pred_Pri2)
	...
	= (thread_info[pred_thread].fetchable <= sim_cycle); 
    }
  ...
  for (cache_lines_left = fetch_cache_lines;
	...
	break;
	  ...
	  && fetch_pri_pol != Pred_Pri2)
	lines_per_thread 
	  = MAX(1, fetch_cache_lines >> mylog2(num_active_forks+1));
      ...
      if (max_cache_lines)
	{
	  ...
	  dassert(fetch_pri_pol != Simple_RR && fetch_pri_pol != Pred_RR
		  && fetch_pri_pol != Pred_Pri2);  
      ...
      if (fetch_pred_pri || fetch_ruu_pri)
	{
	  ...
	  if (lines_per_thread > cache_lines_left
	      || lines_per_thread > max_cache_lines)
...
sim_main(void)
{
  ...
  for (;;)
	...
	if ((sim_num_insn >= (num_prime_insn + num_fullsim_insn))
	    || sim_exit_now)
	  exit_now(0);
	...
	assert(thread_info[INIT_THREAD].valid == TRUE);
      ...
      assert(cache_dl1_ports_reserved <= cache_dl1_ports
	     && cache_dl1_ports_used <= cache_dl1_ports);
